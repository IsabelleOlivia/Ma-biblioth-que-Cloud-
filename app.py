...
def emprunter_livre():
    ...
    return jsonify({'message': 'Livre Emprunter avec succès'})
...
def retourner_livre(livre_id):
    ...
    c.execute(''' UPDATE emprunts
                SET date_retour = ? WHERE id_livre = ? 
                AND date_retour IS NULL''', (date_retour, livre_id))
        
    conn.commit()
    conn.close()

    return jsonify({'message': 'Livre retouner avec succès'})
